...
or you can test with the onw included.
     ...
     docker run --name ssp-default -p 443:443 cirrusid/simplesamlphp:v2.4.5
...
docker run --name ssp-env \
  ...
  -e SSP_APACHE_ALIAS=altinstall/ \
   -p 443:443 cirrusid/simplesamlphp:v2.4.5
```
...
docker run  \
   ...
   cirrusid/simplesamlphp:v2.4.5 -s  /tmp/metadata/example.xml
```
